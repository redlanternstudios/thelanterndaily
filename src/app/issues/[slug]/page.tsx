...
const RELATED = [
  { slug: "memory-agents-architecture", title: "Memory, Agents, Architecture", excerpt: "", date: "Jun 14, 2026", tier: "free" as const, category: "AI Systems" },
  { slug: "what-operators-shipping", title: "What Operators Are Actually Shipping", excerpt: "", date: "Jun 15, 2026", tier: "premium" as const, category: "Product Calls" },
];
      ...
      <div className="page" style={{ paddingTop: "88px" }}>
        ...
        <section className="px-4 sm:px-6 md:px-12 py-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-[1px]" style={{ background: "var(--border)" }}>
            {RELATED.map((item) => (
              <IssueCard key={item.slug} slug={item.slug} title={item.title} excerpt={item.excerpt} date={item.date} tier={item.tier} category={item.category} />
            ))}
          </div>
          <div className="mt-8">
